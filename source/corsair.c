...
struct userInputCorsair {
	    ...
	    int wrongUserInputBool; // Determines if the user input in terminal
				    // is correct or not (correct = 0, incorrect
				    // >0)

	    int helpBool; // Determines if -f is set in order to print help

	    int pFuncBool;    // Determines if the pFunc is called (-p flag)
	    char pPath[2500]; // The path for the -p flag opiton will be here
			      // stored

	    int fFuncBool; // Determines if the fFunc is called (-f flag)
	    char
		pemFileOne4fFunc[2500]; // path for pem file One for function -f
	    char
		pemFileTwo4fFunc[2500]; // path for pem file One for function -f
	    char
		binFileOne4fFunc[2500]; // path for bin file One for function -f
	    char
		binFileTwo4fFunc[2500]; // path for pem file One for function -f

	    int cFuncBool; // Determines if -C function is called (-c flag)
	    char BN_One_cFunc[3000]; // The string corresponding to the first
				     // Bignum will be saved here
	    char BN_Two_cFunc[3000]; // The string corresponding to the two
				     // Bignum will be saved here

	    int xFuncBool;    // Determines if X function is called (flag -x)
	    int number_xFunc; // Determines the number of files to create
			      // (Cannot be 0)
	    ...
	    int outputPathBool;	   // Determines if a output path is given (flag
				   // -o)
...
int SetInputUser(int argc, char *argv[], UserInputCorsair *mainInput)
{
	    ...
	    for (index = 1; index < argc; index++) {
			...
			if ((argv[index][0] == '-') && (FlagExpected == 1)) {
				    ...
				    // For all characters of the argument in
				    // position [index]
				    for (indexArg = 1; indexArg < lengthArgAux;
					 indexArg++) {
						// Find the char in position
						// [indexArg] in order to
						// compare. Equal is allowed
						// because it is not an string,
						// it is just a char
						charArgLetter =
						    argv[index][indexArg];
	    ...
	    */
	    ...
	    if (mainInput->pFuncBool == 1) {
			if (dir_exists(mainInput->pPath) == 0) {
				    printf("The given path with the flag optin "
					   "-p : %s , "
					   "does NOT exists.\n",
					   mainInput->pPath);
	    ...
	    if (mainInput->fFuncBool == 1) {
			...
			if (file1Exists == 0 || file2Exists == 0 ||
			    file3Exists == 0 || file4Exists == 0) {
				    printf("Some or all of the given files "
					   "with the flag "
	    ...
	    if (mainInput->cFuncBool == 1) {
			...
			numberRight1 =
			    CheckJustNumbers(mainInput->BN_One_cFunc);
			numberRight2 =
			    CheckJustNumbers(mainInput->BN_Two_cFunc);

			if (numberRight1 == 0 || numberRight2 == 0) {
				    printf("Some of the numbers given to the "
					   "function with "
					   "flag -c contain no number ASCII "
					   "characters.\n");
				    printf("Just the characters contained in "
					   "the following "
	    ...
	    if (mainInput->xFuncBool == 1) {
			if (mainInput->number_xFunc < 1) {
				    printf("The number given to the function "
					   "with flag -x "
	    ...
	    if (mainInput->helpBool == 0) {
			...
			if (mainInput->outputPathBool == 0) {
				    printf("No output path is given by the "
					   "user. Setting "
					   "output path to standard path as "
					   "./CorsairOutputFiles/\n");
				    strncpy(mainInput->outputPath,
					    "./CorsairOutputFiles/",
			...
			if (mainInput->outputPath[0] == '.') {
				    // FixMe - ToDo - Fix For relative paths
				    // starting by ../
				    strncat(mainInput->outputPathAbs,
					    mainInput->outputPath + 1,
					    sizeof(mainInput->outputPathAbs));

				    // If output path starts with / absolute
				    // path Windowwwwsss why are you not
				    // workiiing... change / for \\ in
				    // windows... bill gates closing doors..
			} else if (mainInput->outputPath[0] == '/') {

				    strncpy(mainInput->outputPathAbs,
					    mainInput->outputPath,
					    sizeof(mainInput->outputPathAbs));
				    ...
				    printf("The given path could not be "
					   "created. Given "
					   "path = %s\n",
					   mainInput->outputPath);
				    printf("The given path could not be "
			...
			if (dir_exists(mainInput->outputPathAbs) == 1) {
				    printf("The output path already exists. "
					   "WARNING - Some "
					   "files could be removed by the "
					   "program if names "
					   "are equal.\n");
			} else {
				    printf(
					"The selected path %s does not exists. "
					"Therefore it will be created.\n",
					mainInput->outputPathAbs);
				    // Crate path
				    pathInt =
					mkdir(mainInput->outputPathAbs, 0777);
				    perror("Warning: \n");
			...
			if (dir_exists(mainInput->outputPathAbs) == 0) {
				    printf("ERROR - The given path could not "
					   "be created! "
					   "Aborting program!\n");
				    printf("The given path could not be "
					   "created. Given "
...
void CreateNCrackableRSAKeys(UserInputCorsair *mainInput)
{
		...
		"Write through the ride", "Cus the time is gonna fly",
		...
		"Forget the taught",	  "on times of draughts."};

	    char listBNs[12][400] = {
		"15567595342746281540680580123176445552529413099110459780408129"
		"8801"
		"23229462021201038929879644983791909905686697372345776409760082"
		"4039"
		"43978001143404214380052718881597897951383102052398140091390319"
		"7794"
		"80769367235203176474886633382313332690429602906927552024084737"
		"2375"
		"765407888803591706810866429593206499159067373",
		"13673648190200445455462255490606487501877893379014454948459826"
		"6156"
		"51039341711074633017010630135966203762232891944436698188278696"
		"1585"
		"71797924301978711872930345653254751720088879899384859922424133"
		"0232"
		"24172163119203647281885212606894525804679762517033027697809306"
		"5898"
		"978936626408010420414304052491164235194189371",
		"15816210396544426215163637809417617511465546534746547878628335"
		"5070"
		"69885707905520361055683056314797211784680615703741996374599635"
		"0596"
		"14426975362825298346810922467550827918758628885563180679799712"
		"6227"
		"92677291508690224001473534394107725228485682874375637689833972"
		"6992"
		"877379358892589997279535629526948347866421981",
		"17237700734290213499179880057470800800122999619558986857532370"
		"6535"
		"19707811606663404585887378748837004397591167164498576526887016"
		"3814"
		"38080766524780043919562851616373340110326382527977521886940714"
		"8518"
		"02220956061475738963181074835432760480177965426240670605257945"
		"6614"
		"384524686240826603559291914632687762523877387",
		"13335346484963442291083793722774098216312231101185818903548037"
		"0020"
		"84616610889016385838788035998817581551090560320027868227746964"
		"1814"
		"32430994104512480486039486469858939546594864240751166746511291"
		"1361"
		"34816625333134559525658450884555557693725268786559060960438936"
		"8494"
		"382302505649394660825302370084417884151308251",
		"15875193253285557633742805157159838944695765889940204733033746"
		"2721"
		"16903177951004828032693825833536238534501804226767595200263795"
		"1838"
		"54824371995703256330815316927497190570399642668737592093784730"
		"6516"
		"67045669253130400057034607758767402409278801795371485699717709"
		"6212"
		"210935417013072384629719507813999385736002929",
		"14164177103449576254773010861119518492434377003695661804710701"
		"5840"
		"95800403852918426073105804676119236096280082582601225522981060"
		"8139"
		"68256576298415558457301990287733433333647255344858275187609620"
		"5800"
		"56202661975188785541164571107959729805228998866853896658270953"
		"4813"
		"679917743849861774368450909066733303358754281",
		"16247434214714378811442229903555598888291454541127176624380483"
		"0514"
		"11243559366225083960212208807260154713506815170167650270924718"
		"9600"
		"34897069514315787928066275737611668734193472701376225437350688"
		"1053"
		"47305708002523394089629146308203809089602659937746701152235169"
		"6078"
		"894999303028818987549272891212876149208168351",
		"15651992289669416536578351093262307178738256037617460614775342"
		"4435"
		"04044383526002681208468429562828798623639227505589925656563271"
		"6468"
		"34800059853577687182898113566331459898974022756063693568811211"
		"3291"
		"60737179548133072776918257950961961631804699945918853086878678"
		"2605"
		"165318635698272036781813521924616331057164531",
		"16818731252461537886667991110881749639439904935913901844825880"
		"5506"
		"33004635511652901391039303547022602587420744948376914027739294"
		"7287"
		"51280910274078396878970687928812596682233068132958422252636240"
		"1256"
		"01673145852323613798707552216757656249899970964218860414223521"
		"7549"
		"795157686077344441618668759870092715066801483",
		"17292375175787831783083636895072549713288338072439687806627558"
		"0794"
		"93119172384291240477362308552806425090155312720463459932176096"
		"9301"
		"90710308296348047300880124346395588538633863713266875971221335"
		"6801"
		"08233470147167569722072660401872395660108273067162886091049193"
		"7158"
		"780901151897190660870828044701404669440795171",
		"17535937722703751579086001282165840397308777948771921308789961"
		"8931"
		"54692041814502488786502085022904241959095160208293213462045329"
		"0576"
		"59316211358930984735692192714687948979285240543493618306505518"
		"4250"
		"98763776043032723413399685609442214397280834980755114033691599"
		"6630"
	    ...
	    // Undeclared variables from copy save rsa key and save encrypted
	    // file:
	    int timeInt;
	    char *pathName[5000], pathOutputPrivateKey[5000],
		pathOutputPublicKey[5000];
	    ...
	    for (i = 0; i < mainInput->number_xFunc; i++) {
			...
			printf("RAND_MAX = %d     randomNum = %d", RAND_MAX,
			       randomNum);
			printf("indexText = %d     indexBN = %d", indexText,
			       indexBN);

			// Set P -Static NUmber
			memset(strAuxStaticNumber, '\0',
			       sizeof(strAuxStaticNumber));
			...
			int counter = 0;
			while (resInt == 0) {
				    resInt = BN_generate_prime_ex(
					newBN, newBNBitsLengthInt, 1, NULL,
					NULL, NULL);
			...
			// CreateKeyFrom2BNsAndReturnRSA(qBN, pBN, **rsaKey) -
			// return RSA Key created Q, P, rsaKey
			...
			// Create Name and file and store private key, pulic key
			// and bin file using previous rsa Save private and
			// public RSA Keys with timestamp
			...
			strncpy(pathOutputPrivateKey, mainInput->outputPath,
				sizeof(mainInput->outputPath));
			strncat(pathOutputPrivateKey, pathName,
				sizeof(pathName));
			...
			strncpy(pathOutputPublicKey, mainInput->outputPath,
				sizeof(mainInput->outputPath));
			strncat(pathOutputPublicKey, pathName,
				sizeof(pathName));
			...
			ret = PEM_write_bio_RSAPrivateKey(
			    bp_private, rsaKey, NULL, NULL, 0, NULL, NULL);
			...
			if (rsaKey != NULL) {
				    // Dencrypt
				    // Get n from rsa key in order to check that
				    // exists:
					...
					lengthMaxToEncrypt) {
						...
						// Save in file in output path
						// with name of txtFile changing
						// extension from .txt or las
						// .something to .bin
						changeFileExtToBin(
						    &binFileName,
						    sizeof(binFileName),
...
void CreateKeyFrom2BNsAndReturnRSA(BIGNUM *qBN, BIGNUM *pBN, RSA **rsaKey)
{
	    ...
	    if (isPrimeP == 1 && isPrimeQ == 1 && comparedPQ != 0) {
			...
			printf("\nThe conditions are NOT satisfied. isPrimeP = "
			       "%i , "
			       "isPrimeQ = %i , comparedPQ = %i\n",
			       isPrimeP, isPrimeQ, comparedPQ);
	    ...
	    // λ(n) = lcm(p − 1, q − 1) giving phi = (p - 1) * (q - 1);
	    ...
	    */
	    ...
	    if (ret != 1) {
			printf("Error while setting factors n e and d for "
			       "rsaOne\n");
	    }
...
void CreateKeyFrom2BNs(UserInputCorsair *mainInput)
{
	    ...
	    */
	    ...
	    // static const char random_seed_Str[] = "OMG change me if you plan
	    // to use me in the future. ERROR - WARNING!!!";

	    unsigned long myError;

	    char *auxStr;      // To print Big Number (char buffer)
	    char timeStr[200]; // For the name of the keys files
	    int timeInt;       // To store the time before converting it into an
			       // string
	    ...
	    */
	    ...
	    ret = RSA_set0_crt_params(rsaCreated, BN_dup(dmp1Key),
				      BN_dup(dmq1Key), BN_dup(iqmpKey));
	    if (ret != 1) {
			printf("\n\n Error while setting CRT parameters to "
			       "rsaCreated.\n\n");
	    }
	    ...
	    snprintf(pathOutputPrivateKey, sizeof(pathOutputPrivateKey),
		     mainInput->outputPath, sizeof(pathOutputPrivateKey));
	    strncat(pathOutputPrivateKey, pathName,
		    sizeof(pathOutputPrivateKey));
	    strncat(pathOutputPrivateKey, "-privateKey.pem",
		    sizeof(pathOutputPrivateKey));

	    snprintf(pathOutputPublicKey, sizeof(pathOutputPublicKey),
		     mainInput->outputPath, sizeof(pathOutputPublicKey));
	    ...
	    printf("Saving private key into file %s \n", pathOutputPrivateKey);
	    bp_private = BIO_new_file(pathOutputPrivateKey, "w+");
	    ret = PEM_write_bio_RSAPrivateKey(bp_private, rsaCreated, NULL,
					      NULL, 0, NULL, NULL);
...
{
	    // Function in order to return N a E from a PEM file with a RSA
	    // public KEy
	    RSA *rsa = RSA_new();
...
void CrackPrivateKeyFromPublicKeysFinal(UserInputCorsair *mainInput)
{
	    ...
	    size_t sizeLengthOneBin;
	    size_t sizeLengthTwoBin;
	    ...
	    if (dOne) {
			...
			while ((dirOne = readdir(dOne)) != NULL) {
				    ...
				    memcpy(nameOne, dirOne->d_name,
					   sizeof(nameOne));

				    // Initialize name with system path for the
				    // file

				    memset(namePathOne, '\0',
					   sizeof(namePathOne));
				    strncat(namePathOne, mainInput->pPath,
					    sizeof(namePathOne));
				    strncat(namePathOne, nameOne,
					    sizeof(namePathOne));

				    // Check if the string ends with PEM
				    // extension:
				    if ((lenOne = strlen(nameOne)) > 3 &&
					strcmp(nameOne + lenOne - 4, ".pem") ==
					    0) {

						counterFileOne++;

						// get values from PEM file:
						// getNandEFromFile(&nOne,
						// &eOne, namePathOne);
						getNandEFromFile(&nOne, &eOne,
								 namePathOne);

						// Recursivity begans:
						dTwo =
						    opendir(mainInput->pPath);
...
void CrackPrivateKeyFromPublicKeysFiles(UserInputCorsair *mainInput)
{
	    ...
	    int gcdBothInt, gcdCmpInt, unityInt, divOneInt, divTwoInt;
	    ...
	    if (gcdCmpInt > 0) {
			...
			printf("INFO - GCD is Bigger than one, therefore RSA "
			       "can be "
			       "cracked");
			...
			// 4. Algorithm Euclidean EGCD(p,q,n,e) --> d
			// obtainDOneInt = obtainD(dValueOne, pBoth, qOne, nOne,
			// eOne); int BN_sub(BIGNUM *r, const BIGNUM *a, const
			// BIGNUM *b);
			...
			// Calculate the inverse mod in order to obtain d
			dValueOne =
			    BN_mod_inverse(NULL, eOne, TotientOne, CTXAux);
			dValueTwo =
			    BN_mod_inverse(NULL, eTwo, TotientTwo, CTXAux);

			if (dValueOne != NULL && dValueTwo != NULL) {
				    printf("\n");
				    printf("Great! It has been cracked. dValue "
					   "%p \n\n",
					   dValueOne);
				    ...
				    RSA_set0_key(rsaOne, BN_dup(nOne),
						 BN_dup(eOne),
						 BN_dup(dValueOne));
				    RSA_set0_key(rsaTwo, BN_dup(nTwo),
						 BN_dup(eTwo),
						 BN_dup(dValueTwo));
				    ...
				    // Set the factors for the rsa key
				    ret = RSA_set0_factors(
					rsaOne, BN_dup(pBoth), BN_dup(qOne));
				    if (ret != 1) {
						printf("Error while setting "
						       "factors for rsaOne\n");
				    }
				    ret = RSA_set0_factors(
					rsaTwo, BN_dup(pBoth), BN_dup(qTwo));
				    if (ret != 1) {
						printf("Error while setting "
						       "factors for rsaOne\n");
				    }

				    ret = BN_mod(dmp1One, dValueOne, pMinusBoth,
						 CTXAux);
				    ret = BN_mod(dmq1One, dValueOne, qMinusOne,
						 CTXAux);
				    iqmpOne = BN_mod_inverse(NULL, qOne, pBoth,
							     CTXAux);
				    ...
				    ret = RSA_set0_crt_params(
					rsaOne, BN_dup(dmp1One),
					BN_dup(dmq1One), BN_dup(iqmpOne));
				    if (ret != 1) {
						printf("\n\n Error while "
						       "setting CRT "
						       "parameters to "
				    ...
				    ret = BN_mod(dmp1Two, dValueTwo, pMinusBoth,
						 CTXAux);
				    ret = BN_mod(dmq1Two, dValueTwo, qMinusTwo,
						 CTXAux);
				    iqmpTwo = BN_mod_inverse(NULL, qTwo, pBoth,
							     CTXAux);
				    ...
				    ret = RSA_set0_crt_params(
					rsaTwo, BN_dup(dmp1Two),
					BN_dup(dmq1Two), BN_dup(iqmpTwo));
				    if (ret != 1) {
						printf("\n\n Error while "
						       "setting CRT "
						       "parameters to "
				    ...
				    // File Buffering for file BIN two START
				    fileOneBin =
					fopen(mainInput->binFileOne4fFunc, "r");
				    if (fileOneBin) {
				    ...
				    // File Buffering for file BIN two
				    fileTwoBin =
					fopen(mainInput->binFileTwo4fFunc, "r");
				    if (fileTwoBin) {
				    ...
				    sizeOneDecrypted = RSA_private_decrypt(
					lengthOneBin, bufferOneBin, toOne,
					rsaOne, RSA_PKCS1_PADDING);
				    printf("\n Message Decrypted One = %.*s \n",
					   sizeOneDecrypted, toOne);

				    sizeTwoDecrypted = RSA_private_decrypt(
					lengthTwoBin, bufferTwoBin, toTwo,
					rsaTwo, RSA_PKCS1_PADDING);
				    printf("\n Message Decrypted Two = %.*s \n",
					   sizeTwoDecrypted, toTwo);

				    // ToDo Version 0.001
				    // Here save .pem private key in file for
				    // fileOne

				    // File One
				    snprintf(filePrivateKeyOnePath,
				    ...
				    snprintf(filePrivateKeyOnePathPem,
					     sizeof(filePrivateKeyOnePathPem),
					     filePrivateKeyOnePath,
					     sizeof(filePrivateKeyOnePathPem));
				    strncat(filePrivateKeyOnePathPem,
					    "_privateKey.pem",
					    sizeof(filePrivateKeyOnePathPem));
				    ...
				    BIO *bp_private = NULL;
				    // 3. save private key
				    bp_private = BIO_new_file(
					filePrivateKeyOnePathPem, "w+");
				    ret = PEM_write_bio_RSAPrivateKey(
					bp_private, rsaOne, NULL, NULL, 0, NULL,
					NULL);
				    BIO_free(bp_private);

				    snprintf(fileMessageDecodedPathOne,
					     sizeof(fileMessageDecodedPathOne),
					     filePrivateKeyOnePath,
					     sizeof(fileMessageDecodedPathOne));
				    strncat(fileMessageDecodedPathOne,
					    "_message.txt",
					    sizeof(fileMessageDecodedPathOne));

				    fileMessageOne =
					fopen(fileMessageDecodedPathOne, "w");

				    fprintf(fileMessageOne, "%.*s",
					    sizeOneDecrypted, toOne);
				    fclose(fileMessageOne);

				    // File Two
				    snprintf(filePrivateKeyTwoPath,
				    ...
				    snprintf(filePrivateKeyTwoPathPem,
					     sizeof(filePrivateKeyTwoPathPem),
					     filePrivateKeyTwoPath,
					     sizeof(filePrivateKeyTwoPathPem));
				    strncat(filePrivateKeyTwoPathPem,
					    "_privateKey.pem",
					    sizeof(filePrivateKeyTwoPathPem));

				    snprintf(fileMessageDecodedPathTwo,
					     sizeof(fileMessageDecodedPathTwo),
					     filePrivateKeyTwoPath,
					     sizeof(fileMessageDecodedPathTwo));
				    strncat(fileMessageDecodedPathTwo,
					    "_message.txt",
					    sizeof(fileMessageDecodedPathTwo));

				    fileMessageTwo =
					fopen(fileMessageDecodedPathTwo, "w");

				    fprintf(fileMessageTwo, "%.*s",
					    sizeTwoDecrypted, toTwo);
				    fclose(fileMessageTwo);

				    // 3. save private key
				    bp_private = BIO_new_file(
					filePrivateKeyTwoPathPem, "w+");
				    ret = PEM_write_bio_RSAPrivateKey(
					bp_private, rsaTwo, NULL, NULL, 0, NULL,
					NULL);
				    BIO_free(bp_private);
...
void CreateNRSAKeys(UserInputCorsair *mainInput)
{
	    ...
	    for (counter = 0; counter < numberKeys; counter++) {
			...
			int bits =
			    2048; // Length of the number n (n=p*q) in bits
			...
			if (ret == 1) {
				    ...
				    rsaKey = RSA_new();
				    ret = RSA_generate_key_ex(rsaKey, bits,
							      BN_e, NULL);
				    ...
				    if (ret == 1) {
						...
						myCRT = BN_mod_inverse(
						    myCRT, q_BN, p_BN, auxCTX);
						...
						// 2. save public key
						bp_public = BIO_new_file(
						    auxPathPublicKey, "w+");
						ret = PEM_write_bio_RSA_PUBKEY(
						    bp_public, rsaKey);
						BIO_free_all(bp_public);
...
void DecryptBinFile(UserInputCorsair *mainInput)
{
	    ...
	    // *enc, unsigned char *kstr, int klen, pem_password_cb *cb, void
	    // *u); RSA *PEM_read_RSAPrivateKey(FILE *fp, RSA **x,
	    ...
	    if (privateKeyFile != NULL) {
			...
			rsaKeyPrivate = PEM_read_RSAPrivateKey(
			    privateKeyFile, rsaAux, NULL, NULL);
			...
			if (rsaKeyPrivate != NULL) {
				    ...
				    sizeOneDecrypted = RSA_private_decrypt(
					lengthOneBin, bufferOneBin, toOne,
					rsaKeyPrivate, RSA_PKCS1_PADDING);
...
void EncryptASCIIFile(UserInputCorsair *mainInput)
{
	    ...
	    if (publicKeyFile != NULL) {
			...
			// rsaKeyPublic = PEM_read_RSAPrivateKey(privateKeyFile,
			// rsaAux, NULL, NULL); ToDo - Version 0.002 - Add
			// Password to following function (3rd parameter)
			rsaKeyPublic = PEM_read_RSAPublicKey(
			    publicKeyFile, rsaAux, NULL, NULL);
			...
			if (rsaKeyPublic != NULL) {
				    ...
				    sizeOneEncrypted = RSA_public_encrypt(
					lengthOneTxt, bufferOneTxt, toOne,
					rsaKeyPublic, RSA_PKCS1_PADDING);
				    printf("Message Encrypted = %.*s \n",
					   sizeOneEncrypted, toOne);

				    // Save in file in output path with name of
				    // txtFile changing extension from .txt or
				    // las .something to .bin
			...
			char fileNameComplete[])
{
	    ...
	    if (lastDotPosPtr == NULL) {
			...
			strncat(outputName, fileNameComplete,
				strlen(fileNameComplete));
			strncat(outputName, ".bin", 4);

	    } else {
			memset(outputName, '\0', sizeOutputName);
			if ((lastDotPosPtr - fileNameComplete) >
			    sizeOutputName) {
				    printf("ERROR - changeFileExtToBin - "
					   "sizeof(outputName) = %zu \n",
					   sizeOutputName);
			} else {
				    strncat(outputName, fileNameComplete,
...
int main(int argc, char *argv[])
{
	    ...
	    -p (path)   --> The main function. Reads all .pem files in the -p
	    path and tries to crack the private password with other .pem files
	    in the folder. If the provate key is cracked it decodes the .bin
	    file with equal name as the private key ".pem" file Example of files
	    in the folder 1.pem 2.pem 1.bin 2.bin If the key of "1.pem" and
	    "2.pem" was cracked, the program decrypts the files 2.bin and 2.bin.
	    It also generates a .pem file for each public key cracked with the
	    respective private key. This file will be named after the file
	    containing the public key, the key type, the program name and the
	    version of the program used. As example if 1.pem and 2.pem files
	    containing a public key where cracked, two files will be created
	    containing the private key. Those files will be named for corsair
	    version 0.001: 1_privateKey_Corsair_0-001.pem
			  2_privateKey_Corsair_0-001.pem
			The function also saves the decoded message in .txt
	    files, which will be named as: 1_decryptedMessage_Corsair_0-001.txt
			  2_decryptedMessage_Corsair_0-001.txt

	    -f (file1.pem file2.pem file1.bin file2.bin)  --> The flag -f does
	    the same as the flag -p but for 4 specific files. It does not read
	    recursively a path looking for files to crack. This flag allows
	    working with files which are not named for the use of the function
	    used for -p It creates the same files as the function used with the
	    -p flag (if key was cracked)

	    -C ("BIGNUM IN ASCII" "BIGNUM IN ASCII")       --> The flag C
	    calculate the RSA keys for the two BIGNUMs give in ASCII format and
	    generates 2 key .pem files. One for the public key and other for the
	    private key. Remember that the numbers need to:
								  1. Be integers
	    (No Float are allowed)
								  2. Be Primes
								  3. Not be the
	    same number

	    -o (outputPath)                                 --> Specifies the
	    ouput path where the output files will be stored. If not output path
	    is specified the relative path "./CorsairOutputV0-001/" will be used
							       [WARNING] The
	    program creates the path if it does NOT exist, but does NOT check
	    for file "collision". Therefore some files in the ouput folder could
	    be deleted.

	    -h        --> Show the help of the program into terminal.

	    -x (N)       --> Generate N "NOT Random" public keys, private keys
	    and encrypted messages in order to check the functionability of the
	    -p path function. (USED FOR DEBUGGING and TESTING)

	    -g (N)      --> Generate N "YES Random" public keys, private keys
	    and encrypted messages in order to check the functionability of the
	    -d path function.

	    -d (private_key_file.pem encrypted_file.bin)   --> Tries to decrypt
	    the ecrypted_file.bin using the private key contained into the
	    ...
	    - Input Read Arguments Function With Parameters stored into Struct
	    and check consistency of gieven parameters
	    - Help Print Function [DONE]
	    - Create Output Folder if needed [DONE]
	    - -p Function [DONE]
	      - Clean
	      - Create .pem files and save into output with name public key
	    files
	      - Create .txt files and save into output path with decrypted
	    message
	      - Free all variables
	    - -f Function [DONE]
	    ...
	    - -x function using the same function as -C
	    -  -g function using the same function as -C
	    - -d function decrypts a file bin using a private key stored in a
	    pem file
	    - Function that creates the private and public key using operations
	    and write theory
	    ...
	    */
	    ...
	    SetInputUser(argc, argv, &mainInput);
	    ...
	    if (mainInput.wrongUserInputBool == 0 && argc > 1) {
			...
			if (mainInput.wrongUserInputBool == 0) {
				    ...
				    // Yeaaaah
				    // Call the functions depending on the
				    // booleans=ints
				    if (mainInput.helpBool == 1) {
						...
						printf(
						    ...
						    "[flag -p]\n");
						CrackPrivateKeyFromPublicKeysFinal(
						    &mainInput);

				    } else if (mainInput.fFuncBool == 1) {
						// Main function for 4 files
						printf("Main function for 4 "
						       "files starting... "
						       "[flag -f]\n");
						CrackPrivateKeyFromPublicKeysFiles(
						    &mainInput);

				    } else if (mainInput.cFuncBool == 1) {
						// Create keys with 2 BIGNUMs p
						// and q
						printf(
						    "Creator for keys "
						    "starting.. [flag -c]\n");
						...
						// 0.002 because of miss of
						// time.");

				    } else if (mainInput.xFuncBool == 1) {
						// Creates N number of keys and
						// bin files encrypted with
						// those keys
						printf("Creating N crackeable "
						       "keys... [flag "
						       "-x]\n");
						printf("ERROR - This function "
						       "will be "
						       ...
						       "0.002 because of "
						       "miss of time.");
						CreateNCrackableRSAKeys(
						    &mainInput);

				    } else if (mainInput.gFuncBool == 1) {
						// Create N number of NO
						// CRACKEBLA keys and bin file
						// encrypted with those keys
						printf("Creating N NO "
						       "CRACKABLE keys.. [flag "
						       "-g]\n");
						// printf("ERROR - This function
						// will be available in version
						// 0.002 because of miss of
						// time. Now it just generates
						// key files (private and
						// public) but no encrypted
						// file");
						CreateNRSAKeys(&mainInput);

				    } else if (mainInput.dFuncBool) {
						// Decrypts the file .bin using
						// the private key contained
						// into the pem file
						printf("Decryption function "
						       "starting... [flag "
						       "-d]\n");
						printf(
						    "ERROR - This function "
						    "will be available "
						    "in version 0.002 because "
						    "of miss of time. "
						    "- It can contain "
						    "ERRORS!\n");
