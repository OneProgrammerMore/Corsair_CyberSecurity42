...
struct userInputCorsair {
  ...
  int wrongUserInputBool; //Determines if the user input in terminal is correct or not (correct = 0, incorrect >0)
  ...
  char BN_One_cFunc [3000]; //The string corresponding to the first Bignum will be saved here
  char BN_Two_cFunc [3000]; //The string corresponding to the two Bignum will be saved here

  int xFuncBool; //Determines if X function is called (flag -x)
  int number_xFunc; //Determines the number of files to create (Cannot be 0)

  int gFuncBool; //Determines if g function is called (flag -g)
  int number_gFunc; // Determines N for g function (number of keys to create)
...
const char helpStrGlobal[] = "corsair is a program used to crack RSA passwords and work with RSA keys. \n\
\n\
...
int SetInputUser(int argc, char *argv[], UserInputCorsair *mainInput){
  ...
  //It starts with 1 as the first argument in the array is the command/program executed
  int index = 1;
  int indexArg = 1;
  char argLetter;
  ...
  for (index =1; index<argc; index++){
    ...
    if((argv[index][0] == '-') && (FlagExpected == 1)){
      ...
      //For all characters of the argument in position [index]
      for(indexArg = 1; indexArg<lengthArgAux; indexArg++){
        //Find the char in position [indexArg] in order to compare.
        //Equal is allowed because it is not an string, it is just a char
      ...
      //-p path
        ...
        strncpy(mainInput->pPath, argv[index], sizeof(mainInput->pPath));
      ...
      //-f file x 4
        ...
        if(counterNoFlag == 0){
          ...
          //Check consistenci of arguments (at least for arguments more given for this flag)
          //Check if 4 or 3
          if((argc - index )  < 4 ){
            printf("Too few files are given for the flag = %c \n", charArgLetter);
            mainInput->wrongUserInputBool = 1;
          }

          strncpy(mainInput->pemFileOne4fFunc, argv[index], sizeof(mainInput->pemFileOne4fFunc));
        }else if(counterNoFlag == 1){
          strncpy(mainInput->pemFileTwo4fFunc, argv[index], sizeof(mainInput->pemFileTwo4fFunc));
        }else if(counterNoFlag == 2){
          strncpy(mainInput->binFileOne4fFunc, argv[index], sizeof(mainInput->binFileOne4fFunc));
        }else if(counterNoFlag == 3){
          strncpy(mainInput->binFileTwo4fFunc, argv[index], sizeof(mainInput->binFileTwo4fFunc));
          noMoreFlags = 0;
          FlagExpected = 1;
        }else{
      ...
      //-C BN BN
        ...
        if(counterNoFlag == 0){
          ...
          strncpy(mainInput->BN_One_cFunc, argv[index], sizeof(mainInput->BN_One_cFunc));

        }else if(counterNoFlag == 1){
          strncpy(mainInput->BN_Two_cFunc, argv[index], sizeof(mainInput->BN_Two_cFunc));
          noMoreFlags = 0;
          FlagExpected = 1;
        }else{
          printf("To many arguments for the flag = %c \n",charArgLetter);
      ...
      //-o output path
        ...
        strncpy(mainInput->outputPath, argv[index], sizeof(mainInput->outputPath));
        noMoreFlags = 0;
        newArgumentExpected = 1;
        FlagExpected = 1;
      ...
      //-x N
        ...
        if(intAux > 0){
          ...
          printf("The argument given for the flag = %c MUST be an Integer higher or equal than 1. \n");
          mainInput->wrongUserInputBool = 1;
        }
      ...
      //-d file1.pem file1.bin
        ...
        if(counterNoFlag == 0){
          ...
          //Check if 1 or 2
          if((argc - index)  < 2 ){
            printf("Too few files ae given for the flag = %c \n", charArgLetter);
            mainInput->wrongUserInputBool = 1;
          }

          strncpy(mainInput->dPrivateKeyFile, argv[index], sizeof(mainInput->dPrivateKeyFile));
        }else if(counterNoFlag == 1){
          strncpy(mainInput->dBinFile, argv[index], sizeof(mainInput->dBinFile));
          noMoreFlags = 0;
          FlagExpected = 1;
        }else{
          printf("To many arguments for the flag = %c \n",charArgLetter);
      ...
      //-d file1.pem file1.bin
        ...
        if(counterNoFlag == 0){
          ...
          strncpy(mainInput->eEncryptPublicKey, argv[index], sizeof(mainInput->eEncryptPublicKey));
        }else if(counterNoFlag == 1){
          strncpy(mainInput->eEncryptTextFile, argv[index], sizeof(mainInput->eEncryptTextFile));
          noMoreFlags = 0;
          FlagExpected = 1;
        }else{
          printf("To many arguments for the flag = %c \n",charArgLetter);
...
void CheckIntegrityProgramVariables(UserInputCorsair *mainInput){
  /*CheckIntegrityProgramVariables is a function that checks the input variables in order to know if they are correct
  It checks:
  ...
  */
  ...
  if(mainInput->pFuncBool == 1){
    if(dir_exists(mainInput->pPath) == 0){
      printf("The given path with the flag optin -p : %s , does NOT exists.\n",mainInput->pPath);
      mainInput->wrongUserInputBool = 1;
    }
  }
  ...
  if(mainInput->fFuncBool == 1){
    ...
    if(file1Exists == 0 ||file2Exists == 0 ||file3Exists == 0 ||file4Exists == 0  ){
      printf("Some or all of the given files with the flag optin -f , do NOT exists.\n");
      mainInput->wrongUserInputBool = 1;
    }
  }
  ...
  if(mainInput->cFuncBool == 1){
    ...
    if(numberRight1 == 0 || numberRight2 == 0){
      printf("Some of the numbers given to the function with flag -c contain no number ASCII characters.\n");
      printf("Just the characters contained in the following string are accepted: 1234567890\n");
      mainInput->wrongUserInputBool = 1;
    }
  ...
  if(mainInput->xFuncBool == 1){
    if(mainInput->number_xFunc < 1){
      printf("The number given to the function with flag -x must be higher than 0\n");
      mainInput->wrongUserInputBool = 1;
    }
  }
  ...
  if(mainInput->gFuncBool == 1){
    if(mainInput->number_gFunc < 1){
      printf("The number given to the function with flag -g must be higher than 0\n");
      mainInput->wrongUserInputBool = 1;
    }
  }
  ...
  if(mainInput->dFuncBool == 1){
    ...
    if(file1Exists == 0 || file2Exists == 0 ){
      printf("Some or all of the given files with the flag option -d , do NOT exists.\n");
      mainInput->wrongUserInputBool = 1;
    }
  }
  ...
  if(getcwd(mainInput->currentDir,sizeof(mainInput->currentDir)) == NULL){
    mainInput->wrongUserInputBool = 1;
  }else{
    strncpy(mainInput->outputPathAbs,mainInput->currentDir,sizeof(mainInput->outputPathAbs));
  }
  ...
  if(mainInput->helpBool == 0){
    ...
    if(mainInput->outputPathBool == 0){
      printf("No output path is given by the user. Setting output path to standard path as ./CorsairOutputFiles/\n");
      strncpy(mainInput->outputPath, "./CorsairOutputFiles/",sizeof(mainInput->outputPath));
    }
    ...
    //Windowwwwsss why are you not workiiing... change / for \\ in windows... bill gates closing doors.. 
    }else if(mainInput->outputPath[0] == '/'){

      strncpy(mainInput->outputPathAbs,mainInput->outputPath,sizeof(mainInput->outputPathAbs));
      printf("Setting path for output as %s \n",mainInput->outputPathAbs);
    
    //Wrong path
    }else {
      printf("The given path could not be created. Given path = %s\n",mainInput->outputPath );
      printf("The given path could not be created. Given path = %s\n",mainInput->outputPathAbs );
      mainInput->wrongUserInputBool = 1;
    ...
    if(dir_exists(mainInput->outputPathAbs) == 1){
      printf("The output path already exists. WARNING - Some files could be removed by the program if names are equal.\n"); 
    }else{
    ...
    if(dir_exists(mainInput->outputPathAbs) == 0){
      printf("ERROR - The given path could not be created! Aborting program!\n");
      printf("The given path could not be created. Given path Abs = %s \n",mainInput->outputPathAbs );
...
void CreateNCrackableRSAKeys(UserInputCorsair *mainInput){
	...
	char textToEncrypt[12][100] = {"Bite the byte",
								"Cus the sky is far",
								"Write thorugh the ride",
								"Cus the time is gonna fly",
								"Type the unspoken",
								"Cus the voices are broken",
								"Read the unsaid",
								"Cus much voices faded away",
								"Learn the unknown",
								"on the way to grow",
								"Forget the taught",
								"on time of draughts."};
	...
	char listBNs[12][400] = {"",
							...
							"",
							"",
							"",
							"",
							"",
							"",
	...
	for(i=0; i<mainInput->number_xFunc; i++){
		...
		strncpy(strAuxStaticNumber, listBNs[indexBN], sizeof(listBNs[indexBN]));
		
		
		resInt = BN_dec2bn(&staticBN, strAuxStaticNumber);
		bitsStaticNum = BN_num_bits(staticBN);
		
		newBNBitsLengthDouble = ((double)bitsLength + 1)-(double)bitsStaticNum;
		...
		while(resInt == 0){
			...
			//Test and check add if fails
			//resInt = BN_generate_prime_ex2(newBN, newBNBitsLengthInt, 0, NULL, NULL, auxGENCB, auxCTX);
			resInt = BN_generate_prime_ex(newBN, newBNBitsLengthInt, 1, NULL, NULL, NULL);
			
			if(resInt != 1 && counter==0){
				printf("ERROR - While generating random Bignumber! resint = %d\n", resInt);
		...
		//Edit function in order to (with other name as  CreateKeyFrom2BNsAndReturnRSA)
		//CreateKeyFrom2BNsAndReturnRSA(qBN, pBN, **rsaKey) - return RSA Key created
		//Q, P, rsaKey
		...
		//Create Name and file and store private key, pulic key and bin file using previous rsa
		...
		//strncat(pathOutputPublicKey, "-publicKey.pem", sizeof(pathOutputPublicKey));
		...
		if(rsaKey != NULL){
			...
			strncpy(bufferOneTxt,  textToEncrypt[indexText], strlen(textToEncrypt[indexText]));

			printf("Message to encrypt = %s\n",bufferOneTxt );
			lengthOneTxt = strlen(bufferOneTxt);
			printf("lengthOneTxt = %d\n",lengthOneTxt );
			
			sizeOneEncrypted = RSA_public_encrypt(lengthOneTxt, bufferOneTxt, toOne, rsaKey, RSA_PKCS1_PADDING);
			
			//free(textToEncryptPointer);
			printf("Message Encrypted = %.*s \n",sizeOneEncrypted, toOne);
			printf("sizeOneEncrypted = %d\n",sizeOneEncrypted);
			if(sizeOneEncrypted == -1){
				printf("ERROR - CreateNCrackableRSAKeys - sizeOneEncrypted == -1");
				//Error handling:
				...
				//Save in file in output path with name of txtFile changing extension from .txt or las .something to .bin
				changeFileExtToBin(&binFileName, sizeof(binFileName), basename(pathOutputPublicKey));

				printf("sizeof(binFileNameAndPath) = %d\n",sizeof(binFileNameAndPath));
				memset(binFileNameAndPath, '\0', sizeof(binFileNameAndPath));
				...
				if(fileOneBin == NULL){
					...
					fwrite(toOne, 1, sizeOneEncrypted, fileOneBin);
					fclose(fileOneBin);
				}
			}
			free(toOne);
...
void CreateKeyFrom2BNsAndReturnRSA(BIGNUM *qBN, BIGNUM *pBN, RSA **rsaKey){
  ...
  */
  ...
  //static const char random_seed_Str[] = "OMG change me if you plan to use me in the future. ERROR - WARNING!!!";
  ...
  if(isPrimeP == 1 & isPrimeQ == 1 & comparedPQ != 0){
    printf("\nThe conditions are satisfied. isPrimeP = %i , isPrimeQ = %i , comparedPQ = %i\n", isPrimeP, isPrimeQ, comparedPQ);
  }else{
  ...
  */
  ...
  ret = RSA_set0_crt_params(*rsaKey, BN_dup(dmp1Key),BN_dup(dmq1Key), BN_dup(iqmpKey));
  ...
  if(ret != 1){
	...
	BN_free(nBN); BN_free(eBN); BN_free(dBN);
	//BN_free(pBN); BN_free(qBN);
	BN_free(dmp1Key); BN_free(dmq1Key); BN_free(iqmpKey);
	
	BN_CTX_free(CTXAux);
	
	BN_free(numPMinusBN); BN_free(numQMinusBN); BN_free(phiBN);
	BN_free(gcd_eAndTotientBN); BN_free(kBN); BN_free(kTimesPhiBN);
	BN_free(kTimesPhiPlusOneBN); BN_free(kTimesPhiPlusOneDivEBN); BN_free(zeroBN);
	BN_free(oneBN);
...
void CreateKeyFrom2BNs(UserInputCorsair *mainInput){
  ...
  */
  ...
  ret = RSA_set0_crt_params(rsaCreated, BN_dup(dmp1Key),BN_dup(dmq1Key), BN_dup(iqmpKey));
  if(ret != 1){
  ...
  strncpy(pathName, timeStr, sizeof(pathName));

  strncpy(pathOutputPrivateKey, mainInput->outputPath, sizeof(pathOutputPrivateKey));
  ...
  strncpy(pathOutputPublicKey, mainInput->outputPath, sizeof(pathOutputPublicKey));
  ...
  printf("Saving private key into file %s \n", pathOutputPrivateKey);
  bp_private = BIO_new_file(pathOutputPrivateKey, "w+");
  ret = PEM_write_bio_RSAPrivateKey(bp_private, rsaCreated, NULL, NULL, 0, NULL, NULL);
  ...
  //START ------------------------------------------------------
  BN_free(nBN); BN_free(pBN); BN_free(qBN); BN_free(numPMinusBN); BN_free(numQMinusBN); BN_free(phiBN);
  BN_free(eBN); BN_free(gcd_eAndTotientBN); BN_free(dBN); BN_free(kBN); BN_free(kTimesPhiBN); BN_free(kTimesPhiPlusOneBN);
  BN_free(kTimesPhiPlusOneDivEBN); BN_free(zeroBN); BN_free(oneBN);
  
  //ToDo- DUP BN FROM RSA
  RSA_free(rsaCreated);
  
  BN_free(eRSA); BN_free(nRSA); BN_free(dRSA);
  BN_free(dmp1Key); BN_free(dmq1Key); BN_free(iqmpKey); 
  BN_CTX_free(CTXAux);
...
void CrackPrivateKeyFromPublicKeysFinal(UserInputCorsair  *mainInput){
  ...
  char *nOneStr, *eOneStr, *nTwoStr, *eTwoStr, *TotientOneStr, *qMinusOneStr, *pMinusBothStr, *qOneStr, *pBothStr, *gcdBNStr, *unityBNStr;
  ...
  if(dOne){
    ...
    while( (dirOne = readdir(dOne)) != NULL){
      ...
      strncat(namePathOne, mainInput->pPath, sizeof(namePathOne));
      strncat(namePathOne, nameOne, sizeof(namePathOne));


      //Check if the string ends with PEM extension:
      if( (lenOne = strlen(nameOne)) > 3 && strcmp(nameOne + lenOne - 4, ".pem")==0){
        ...
        if(dTwo){
          ...
          while( (dirTwo = readdir(dTwo)) != NULL){

            //nameOne = dirTwo->d_name;

            memset(nameTwo, '\0', sizeof(nameTwo));
            memcpy(nameTwo, dirTwo->d_name, strlen(dirTwo->d_name));

            memset(namePathTwo, '\0', sizeof(namePathTwo));

            strncat(namePathTwo, mainInput->pPath, sizeof(namePathTwo));
            strncat(namePathTwo, nameTwo, sizeof(namePathTwo));

            //Check that the file Two is not the same as the file One:
            if(strcmp(nameOne,nameTwo) != 0){

              //Check if the string ends with PEM extension in second File
              if( (lenTwo = strlen(nameTwo)) > 3 && strcmp(nameTwo + lenTwo - 4, ".pem")==0){

                counterFile++;

                getNandEFromFile(&nTwo, &eTwo, namePathTwo);

				//if nTwo and eTwo exists:
				if(nTwo!= NULL && eTwo != NULL && nOne!=NULL && eOne!=NULL){
					...
					//printf("gcdBN = %s     unityBN = %s    counterFile = %d nameOne = %s    nameTwo = %s    counterFileOne = %d    counterAux = %d \n", gcdBNStr, unityBNStr, counterFile, nameOne, nameTwo, counterFileOne, counterAux);
					...
					if(gcdCmpInt > 0){
					  ...
					  totientOneInt = BN_mul(TotientOne, qMinusOne, pMinusBoth, CTXAux);
					  //free(CTXAux);
					  totientTwoInt = BN_mul(TotientTwo, qMinusTwo, pMinusBoth, CTXAux);
					  //free(CTXAux);
					  ...
					  dValueOne = BN_mod_inverse(dValueOne, eOne, TotientOne, CTXAux);
					  dValueTwo = BN_mod_inverse(dValueTwo, eTwo, TotientTwo, CTXAux);

					  if(dValueOne != NULL && dValueOne != NULL){
						printf("Great! It has been cracked. \n");
						...
						RSA_set0_key(rsaOne, BN_dup(nOne), BN_dup(eOne), BN_dup(dValueOne));
						RSA_set0_key(rsaTwo, BN_dup(nTwo), BN_dup(eTwo), BN_dup(dValueTwo));

						//Set the factors for the rsa key
						ret = RSA_set0_factors(rsaOne, BN_dup(pBoth), BN_dup(qOne));
						if(ret != 1){
						  printf("Error while setting factors for rsaOne\n");
						}
						ret = RSA_set0_factors(rsaTwo, BN_dup(pBoth), BN_dup(qTwo));
						if(ret != 1){
						  printf("Error while setting factors for rsaOne\n");
						}
						...
						//free(CTXAux);
						ret = BN_mod_inverse(iqmpOne, qOne, pBoth, CTXAux);
						//free(CTXAux);

						ret = RSA_set0_crt_params(rsaOne, BN_dup(dmp1One),BN_dup(dmq1One), BN_dup(iqmpOne));
						if(ret != 1){
						...
						*/
						...
						//free(CTXAux);
						ret = BN_mod_inverse(iqmpTwo, qTwo, pBoth, CTXAux);
						//free(CTXAux);

						ret = RSA_set0_crt_params(rsaTwo, BN_dup(dmp1Two),BN_dup(dmq1Two), BN_dup(iqmpTwo));
						if(ret != 1){
					...
					//Recursivity begans:
					dOneBin = opendir(mainInput->pPath);
						
					//First for dirOneBin:
					if(dOneBin){
	
						//For all the files of the directory
						while( (dirOneBin = readdir(dOneBin)) != NULL){
							...
							if( strlen(nameOne)>4 && strlen(nameOneBin)>4){
								//Check if the file name is the same but with .bin extesion
								lenOneBin = strlen(nameOneBin);
								//lenOne = strlen(nameOneBin)
								//Check appropiate extensions for files
								if(strcmp(nameOneBin + lenOneBin - 4, ".bin")==0  && strcmp(nameOne + lenOne - 4, ".pem")==0 ){
									//Check equal name:
									memset(nameOneBinAux, '\0', sizeof(nameOneBinAux));
									mempcpy(nameOneBinAux, nameOneBin, strlen(nameOneBin)-4);
									
									memset(nameOneAux, '\0', sizeof(nameOneAux));
									mempcpy(nameOneAux, nameOne, strlen(nameOne)-4);
									//Check equal name:
									if(strcmp(nameOneAux, nameOneBinAux) == 0){
										break;					
									}
					...
					//Recursivity begans:
					dTwoBin = opendir(mainInput->pPath);
						
					//First for dirOneBin:
					if(dTwoBin){
	
						//For all the files of the directory
						while( (dirTwoBin = readdir(dTwoBin)) != NULL){
							...
							//Check if file name is larger than 4 letters (in order to contain .bin extension)
							if( strlen(nameTwo)>4 && strlen(nameTwoBin)>4){
								//Check if the file name is the same but with .bin extesion
								lenTwoBin = strlen(nameTwoBin);
								//lenTwo = strlen(nameTwoBin)
								//Check appropiate extensions for files
								if(strcmp(nameTwoBin + lenTwoBin - 4, ".bin")==0  && strcmp(nameTwo + lenTwo - 4, ".pem")==0 ){
									//Check equal name:
									memset(nameTwoBinAux, '\0', sizeof(nameTwoBinAux));
									mempcpy(nameTwoBinAux, nameTwoBin, strlen(nameTwoBin)-4);
									
									memset(nameTwoAux, '\0', sizeof(nameTwoAux));
									mempcpy(nameTwoAux, nameTwo, strlen(nameTwo)-4);
									//Check equal name:
									if(strcmp(nameTwoAux, nameTwoBinAux) == 0){			
					...
					printf("nameOne (PEM File) =  %s              ||              nameOneBin (Binary File) =  %s \n", nameOne, nameOneBin);
					printf("nameTwo (PEM File) =  %s              ||              nameTwoBin (Binary File) =  %s \n", nameTwo, nameTwoBin);
					...
					*/
					...
					memset(namePathOneBin, '\0', sizeof(namePathOneBin));
					strncat(namePathOneBin, mainInput->pPath, sizeof(namePathOneBin));
					strncat(namePathOneBin, nameOneBin, sizeof(namePathOneBin));
					...
					*/
					...
					memset(namePathTwoBin, '\0', sizeof(namePathTwoBin));
					strncat(namePathTwoBin, mainInput->pPath,sizeof(namePathTwoBin));
					strncat(namePathTwoBin, nameTwoBin, sizeof(namePathTwoBin));
					

					//FILE BIN TWO name Creation with path
					memset(realOneFileName, '\0', sizeof(realOneFileName));
					strncpy(realOneFileName, nameOneBin, sizeof(realOneFileName));

					memset(realOneFileNamePlusPath, '\0', sizeof(realOneFileNamePlusPath));
					strncpy(realOneFileNamePlusPath, namePathOneBin, sizeof(realOneFileNamePlusPath));

					
					//File Buffering for file BIN two START
					fileOneBin = fopen(namePathOneBin, "r");
					if (fileOneBin)
					{
						...
						if (bufferOneBin)
						{
						fread (bufferOneBin, 1, lengthOneBin, fileOneBin);
						}
						fclose (fileOneBin);
					...
					//FILE BIN TWO name Creation with path
					memset(realTwoFileName, '\0', sizeof(realTwoFileName));
					strncpy(realTwoFileName, nameTwoBin, strlen(nameTwoBin));

					memset(realTwoFileNamePlusPath, '\0', sizeof(realTwoFileNamePlusPath));
					strncpy(realTwoFileNamePlusPath, namePathTwoBin, strlen(realTwoFileNamePlusPath));

					//File Buffering for file BIN two
					fileTwoBin = fopen(namePathTwoBin, "r");
					if (fileTwoBin)
					{
						...
						if (bufferTwoBin)
						{
						fread (bufferTwoBin, 1, lengthTwoBin, fileTwoBin);
						}
						fclose (fileTwoBin);
					}
					...
					sizeOneDecrypted = RSA_private_decrypt(lengthOneBin, bufferOneBin, toOne, rsaOne, RSA_PKCS1_PADDING);
					printf("Message Decrypted One = %.*s \n",sizeOneDecrypted, toOne);

					//printf("BufferOneBin %s \n", bufferOneBin);

					//RSA_private_decrypt(sizeLengthTwoBin, bufferTwoBin, toTwo, rsaTwo, RSA_PKCS1_PADDING);
					sizeTwoDecrypted = RSA_private_decrypt(lengthTwoBin, bufferTwoBin, toTwo, rsaTwo, RSA_PKCS1_PADDING);
					printf("Message Decrypted Two = %.*s \n",sizeTwoDecrypted, toTwo);
					...
					//File One
					strncpy(filePrivateKeyOnePath, mainInput->outputPathAbs, sizeof(filePrivateKeyOnePath));
					strncat(filePrivateKeyOnePath,basename(namePathOne),strlen(basename(namePathOne))-4);
					strncpy(filePrivateKeyOnePathPem,filePrivateKeyOnePath,sizeof(filePrivateKeyOnePathPem));
					strncat(filePrivateKeyOnePathPem,"_privateKey.pem",sizeof(filePrivateKeyOnePathPem));


					//WORKING
					BIO *bp_privateOne = NULL;
					// 3. save private key

					bp_privateOne = BIO_new_file(filePrivateKeyOnePathPem, "w+");
					ret = PEM_write_bio_RSAPrivateKey(bp_privateOne, rsaOne,NULL , NULL, 0, 0, NULL);
					printf("Saving the RSA private key into file %s \n gives ret = %d \n", filePrivateKeyOnePathPem, ret);
					BIO_free(bp_privateOne);


					strncpy(fileMessageDecodedPathOne, filePrivateKeyOnePath, sizeof(fileMessageDecodedPathOne));
					strncat(fileMessageDecodedPathOne,"_message.txt",sizeof(fileMessageDecodedPathOne));
					
					fileMessageOne = fopen(fileMessageDecodedPathOne,"w");
					
					fprintf(fileMessageOne,"%.*s",sizeOneDecrypted,toOne);
					fclose(fileMessageOne);

					//File Two
					strncpy(filePrivateKeyTwoPath, mainInput->outputPathAbs, sizeof(filePrivateKeyTwoPath));
					strncat(filePrivateKeyTwoPath,basename(namePathTwo),strlen(basename(namePathTwo))-4);

					strncpy(filePrivateKeyTwoPathPem,filePrivateKeyTwoPath,sizeof(filePrivateKeyTwoPathPem));
					strncat(filePrivateKeyTwoPathPem,"_privateKey.pem",sizeof(filePrivateKeyTwoPathPem));

					
					//NEW VERSION:
					BIO *bp_privateTwo = NULL;
					// 3. save private key
					bp_privateTwo = BIO_new_file(filePrivateKeyTwoPathPem, "w+");
					ret = PEM_write_bio_RSAPrivateKey(bp_privateTwo, rsaTwo, NULL, NULL, 0, NULL, NULL);
					printf("Saving the RSA private key into file %s \n gives ret = %d \n", filePrivateKeyTwoPathPem, ret);
					BIO_free(bp_privateTwo);


					strncpy(fileMessageDecodedPathTwo, filePrivateKeyTwoPath, sizeof(fileMessageDecodedPathTwo));
					strncat(fileMessageDecodedPathTwo,"_message.txt",sizeof(fileMessageDecodedPathTwo));
					
					fileMessageTwo = fopen(fileMessageDecodedPathTwo,"w");
					
					fprintf(fileMessageTwo,"%.*s",sizeTwoDecrypted,toTwo);
				  ...
				  }//End if gcdInt For when the CGD is bigger than one and the rsa can be cracked

					
				}//END if checking n and e One and Two not NULL
				BN_free(nTwo);
				BN_free(eTwo);
            ...
            }//Check if strings name are not the same end if
...
void CrackPrivateKeyFromPublicKeysFiles(UserInputCorsair  *mainInput){
  ...
  */
  ...
  if(gcdCmpInt > 0){
    ...
    printf("INFO - GCD is Bigger than one, therefore RSA can be cracked");
    ...
    if(dValueOne != NULL && dValueTwo != NULL){
      ...
      if(iqmpOne == NULL){
		printf("ERROR - Not possible to calculate mod inverse of qOne and pBoth!\n");
	  }
      ...
      //File One
      strncpy(filePrivateKeyOnePath, mainInput->outputPathAbs, sizeof(filePrivateKeyOnePath));
      strncat(filePrivateKeyOnePath,basename(mainInput->pemFileOne4fFunc),strlen(basename(mainInput->pemFileOne4fFunc))-4);
      ...
      BIO *bp_private = NULL;
      // 3. save private key
      bp_private = BIO_new_file(filePrivateKeyOnePathPem, "w+");
      ret = PEM_write_bio_RSAPrivateKey(bp_private, rsaOne, NULL, NULL, 0, NULL, NULL);
      BIO_free(bp_private);
      ...
      //File Two
      strncpy(filePrivateKeyTwoPath, mainInput->outputPathAbs, sizeof(filePrivateKeyTwoPath));
      strncat(filePrivateKeyTwoPath,basename(mainInput->pemFileTwo4fFunc),strlen(basename(mainInput->pemFileTwo4fFunc))-4);
      ...
      // 3. save private key
      bp_private = BIO_new_file(filePrivateKeyTwoPathPem, "w+");
      ret = PEM_write_bio_RSAPrivateKey(bp_private, rsaTwo, NULL, NULL, 0, NULL, NULL);
      BIO_free(bp_private);
...
void PrintHelp(){
  printf(helpStrGlobal);
}


void CreateNRSAKeys(UserInputCorsair *mainInput){
  /*Create N RSA keys creates N RSA keys and saves the keys into the output folder*/
  ...
  */
  ...
  for(counter = 0; counter < numberKeys; counter++){
    ...
    if(ret == 1){
      ...
      //ret = RSA_generate_key_ex(rsaKey, bits, BN_dup(BN_e), NULL);
      ...
      if(ret == 1){
        ...
        strncpy(auxPathPrivateKey, mainInput->outputPath, sizeof(auxPathPrivateKey));
        strncat(auxPathPrivateKey, auxName, sizeof(auxPathPrivateKey));
        strncat(auxPathPrivateKey, "privateKey.pem", sizeof(auxPathPrivateKey));

        //Create path for public key
        strncpy(auxPathPublicKey, mainInput->outputPath, sizeof(auxPathPublicKey));
        strncat(auxPathPublicKey, auxName, sizeof(auxPathPublicKey));
        strncat(auxPathPublicKey, "publicKey.pem", sizeof(auxPathPublicKey));



        //Fixme segmentation fault maybe
        //RSA_get0_crt_params(rsaKey, BN_dup(&dmp1), BN_dup(&dmq1), BN_dup(&iqmp));
        //RSA_get0_crt_params(rsaKey, BN_dup(dmp1), BN_dup(dmq1), BN_dup(iqmp));
        ...
        //RSA_get0_factors(rsaKey, BN_dup(&p_BN), BN_dup(&q_BN));
        //RSA_get0_factors(rsaKey, BN_dup(p_BN), BN_dup(q_BN));
        ...
        ////BIGNUM *BN_mod_inverse(BIGNUM *r, BIGNUM *a, const BIGNUM *n, BN_CTX *ctx);
        //modInverseBN = BN_mod_inverse(modInverseBN, remModTotientToEBN, multTotientBN, modInverseCTXBN);

        myCRT = BN_mod_inverse(myCRT, q_BN, p_BN, auxCTX);
        ...
        // 2. save public key
        bp_public = BIO_new_file(auxPathPublicKey, "w+");
        ...
        // 3. save private key
        bp_private = BIO_new_file(auxPathPrivateKey, "w+");
        ret = PEM_write_bio_RSAPrivateKey(bp_private, rsaKey, NULL, NULL, 0, NULL, NULL);
...
void DecryptBinFile(UserInputCorsair *mainInput){
  ...
  if(privateKeyFile != NULL){
    ...
    rsaKeyPrivate = PEM_read_RSAPrivateKey(privateKeyFile, rsaAux, NULL, NULL);
    ...
    if(rsaKeyPrivate != NULL){
      ...
      sizeOneDecrypted = RSA_private_decrypt(lengthOneBin, bufferOneBin, toOne, rsaKeyPrivate, RSA_PKCS1_PADDING);
...
void EncryptASCIIFile(UserInputCorsair *mainInput){
  ...
  if(publicKeyFile != NULL){
    ...
    //rsaKeyPublic = PEM_read_RSAPrivateKey(privateKeyFile, rsaAux, NULL, NULL);
    //ToDo - Version 0.002 - Add Password to following function (3rd parameter)
    rsaKeyPublic = PEM_read_RSAPublicKey(publicKeyFile, rsaAux, NULL, NULL);
    ...
    if(rsaKeyPublic != NULL){
      ...
      if (fileOneTxt)
      {
        ...
        if (bufferOneTxt)
        {
          elem = fread (bufferOneTxt, 1, lengthOneTxt, fileOneTxt);
        }
        fclose (fileOneTxt);
      }//File Buffering for file BIN two END
      ...
      printf("bufferOneTxt = %.*s", lengthOneTxt, bufferOneTxt);
      printf("elem = %d", elem);
      sizeOneEncrypted = RSA_public_encrypt(lengthOneTxt, bufferOneTxt, toOne, rsaKeyPublic, RSA_PKCS1_PADDING);
      printf("Message Encrypted = %.*s \n",sizeOneEncrypted, toOne);
      ...
      if(sizeOneEncrypted == -1){
		  ...
		  //Save in file in output path with name of txtFile changing extension from .txt or las .something to .bin
		  changeFileExtToBin(&binFileName, sizeof(binFileName), basename(txtFile));

		  
		  memset(binFileNameAndPath, '\0', sizeof(binFileNameAndPath));
		  strncat(binFileNameAndPath, mainInput->outputPath, sizeof(binFileNameAndPath));
		  strncat(binFileNameAndPath, binFileName, sizeof(binFileNameAndPath));
...
void changeFileExtToBin(char **outputName, size_t sizeOutputName, char fileNameComplete[]){
	...
	if(lastDotPosPtr == NULL){
		...
		if ((lastDotPosPtr - fileNameComplete) > sizeOutputName){
			printf("ERROR - changeFileExtToBin - sizeof(outputName) = %d \n", sizeOutputName);
		}else{
			strncat(outputName, fileNameComplete, (lastDotPosPtr - fileNameComplete));
			strncat(outputName, ".bin", 4);
		}
	}
	return;
...
int main(int argc, char *argv[]) {
  ...
  -p (path)   --> The main function. Reads all .pem files in the -p path and tries to crack the private password with other .pem files in the folder. If the provate key is cracked it decodes the .bin file 
              with equal name as the private key ".pem" file
              Example of files in the folder
                1.pem
                2.pem 
                1.bin
  ...
  -C ("BIGNUM IN ASCII" "BIGNUM IN ASCII")       --> The flag C calculate the RSA keys for the two BIGNUMs give in ASCII format and generates 2 key .pem files. One for the public key
                                                      and other for the private key.
                                                      Remember that the numbers need to:
                                                        1. Be integers (No Float are allowed)
                                                        2. Be Primes
  ...
  ToDo for Version 0.002
  - H (HASH)   --> Specifies the HASH to be used for all functions 
  - F (file.pem) --> Cracks the public key stored into file.pem using brute Force
    - Shows expected time
  - Clean Code
  ...
  - -x function using the same function as -C
  -  -g function using the same function as -C
  - -d function decrypts a file bin using a private key stored in a pem file
  - Function that creates the private and public key using operations and write theory
    ...
    if(mainInput.wrongUserInputBool == 0 && argc > 1){
      ...
      if(mainInput.wrongUserInputBool == 0){
        ...
        if(mainInput.helpBool == 1){
          ...
          printf("Main function for 4 files starting... [flag -f]\n");
          CrackPrivateKeyFromPublicKeysFiles(&mainInput);
          
        }else if(mainInput.cFuncBool == 1){
          //Create keys with 2 BIGNUMs p and q
          printf("Creator for keys starting.. [flag -c]\n");
          CreateKeyFrom2BNs(&mainInput);
          //printf("ERROR - This function will be available in version 0.002 because of miss of time.");
          
        }else if(mainInput.xFuncBool == 1){
          //Creates N number of keys and bin files encrypted with those keys
          printf("Creating N crackeable keys... [flag -x]\n");
          printf("ERROR - This function will be available in version 0.002 because of miss of time.");
          CreateNCrackableRSAKeys(&mainInput);
          
        }else if(mainInput.gFuncBool == 1){
          //Create N number of NO CRACKEBLA keys and bin file encrypted with those keys
          printf("Creating N NO CRACKABLE keys.. [flag -g]\n");
          //printf("ERROR - This function will be available in version 0.002 because of miss of time. Now it just generates key files (private and public) but no encrypted file");
          CreateNRSAKeys(&mainInput);
          
        }else if(mainInput.dFuncBool){
          //Decrypts the file .bin using the private key contained into the pem file
          printf("Decryption function starting... [flag -d]\n");
          printf("ERROR - This function will be available in version 0.002 because of miss of time. - It can contain ERRORS!\n");
          DecryptBinFile(&mainInput);
          
        }else if(mainInput.eFuncBool){
          //Decrypts the file .bin using the private key contained into the pem file
          printf("Encryption function starting... [flag -e]\n");
          printf("ERROR - This function will be available in version 0.002 because of miss of time. - It can contain ERRORS!\n");
